// Package ir wires protokit's service IR into its source/target factory.
//
// The split follows the one protokit draws: a Source builds a model, a Target
// renders it, and a Registry pairs them. The model here is the service IR
// itself, so every target — Rust, Go, OpenAPI — reads exactly the same thing
// and cannot drift in what it believes the API to be.
package ir
