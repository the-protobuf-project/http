...
func (a *Adapter) newRouteCx(r *http.Request, method route.Method, matched *route.Route, captures map[string]string) *middleware.RouteCx {
	return &middleware.RouteCx{
		...
		Captures: captures,
		Peer:     r.RemoteAddr,
		Metadata: middleware.MetadataFromHeaders(r.Header, a.headers.Incoming),
	}
}
...
func (a *Adapter) runRoute(cx *middleware.RouteCx, selected middleware.Selected) error {
...
func (a *Adapter) runComplete(cx *middleware.CallCx, selected middleware.Selected, outcome middleware.Outcome) {
	for _, hook := range selected.Complete {
		a.safeComplete(hook, cx, outcome)
	}
}

// safeComplete runs one completion hook, containing a panic in it.
func (a *Adapter) safeComplete(hook middleware.CompleteHook, cx *middleware.CallCx, outcome middleware.Outcome) {
	defer func() {
		if recovered := recover(); recovered != nil {
			a.options.Logger.Error("completion hook panicked",
