...
func (h *Handler) serveStream(w http.ResponseWriter, call *Call) *apierr.Error {
	dispatcher, ok := h.dispatch.(StreamDispatcher)
	if !ok {
		failure := h.asAPIError(call.Errorf(apierr.Unimplemented,
			"This method streams its response, which this transcoder was not built to serve."))
		errorReply(failure).Write(w)
		return failure
	}

	writer := stream.NewWriter(call.ResponseCodec.Framing, call.ResponseCodec.ContentType())
	out := &Stream{writer: writer, response: w, encode: h.encodeError}

	err := dispatcher.DispatchStream(call, out)
	if err == nil {
		return h.terminate(w, writer.Finish())
	}
	return h.terminate(w, writer.Fail(h.asAPIError(err), h.encodeError))
}

// terminate acts on how a stream ended, returning the failure if there was one.
func (h *Handler) terminate(w http.ResponseWriter, termination stream.Termination) *apierr.Error {
...
func (h *Handler) encodeError(err *apierr.Error) []byte {
