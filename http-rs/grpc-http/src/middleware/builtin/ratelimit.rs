...
use crate::error::{Code, Detail, GatewayError, QuotaFailure, QuotaViolation, Result, RetryInfo};
...
///
/// The gateway does not implement the counting: a real limit is shared across
...
impl Interceptor for RateLimit {
    ...
    fn on_route(&self, cx: &mut RouteCx<'_>) -> Result<()> {
        ...
        Err(Box::new(
            GatewayError::new(Code::ResourceExhausted, "Quota exceeded for this caller.")
                .with_error_info(
                    "RATE_LIMIT_EXCEEDED",
                    self.domain,
