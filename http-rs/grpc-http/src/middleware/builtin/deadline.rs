...
/// per-method override, then the configured default. A gateway must always have
/// one — an unbounded default turns a single slow backend into connection-pool
/// exhaustion, and by the time that is visible the cause is several layers away.
///
...
impl Deadline {
    ...
    #[must_use]
    pub const fn new(default: Duration) -> Self {
        Self {
            default,
...
impl Interceptor for Deadline {
    ...
    /// backend stops working on a call the gateway has already abandoned.
    ...
    fn on_request(&self, cx: &mut CallCx<'_>) -> Result<()> {
        if cx.expired() {
            return Err(cx.deadline_error("gateway"));
        }
        Ok(())
    }
}
