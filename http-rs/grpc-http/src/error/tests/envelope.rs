...
use super::DOMAIN;
use crate::error::{Code, Detail, FieldViolation, GatewayError};
use http::StatusCode;
use serde_json::json;
...
fn envelope_code_is_the_http_status_not_the_grpc_code() {
    ...
    let err = GatewayError::new(Code::InvalidArgument, "Bad book.").ensure_error_info(DOMAIN);
...
fn method_not_allowed_stays_405_and_carries_allow() {
    ...
    let err = GatewayError::method_not_allowed("PATCH", &["GET", "DELETE"], DOMAIN);
...
fn field_violations_are_reported_together() {
    ...
    let err = GatewayError::invalid_fields(
        vec![
            FieldViolation {
...
fn localized_message_is_preferred_for_the_rendered_message() {
    let err = GatewayError::new(Code::NotFound, "not found")
...
fn from_tonic_status_maps_code_and_seeds_error_info() {
    ...
    let err = GatewayError::from_status(&status, DOMAIN);
