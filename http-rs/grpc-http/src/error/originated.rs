//! Failures the gateway originates itself, before or instead of an RPC.
//!
//! Each carries a reason token from the README §5.4 set, so a caller can
//! tell a gateway rejection from a service one without parsing prose.
...
impl GatewayError {
    ...
    pub fn route_not_found(path: &str, domain: &str) -> Self {
        GatewayError::new(Code::NotFound, format!("No route matches {path:?}.")).with_error_info(
            "ROUTE_NOT_FOUND",
            domain,
            [("path".into(), path.into())],
        )
    ...
    pub fn invalid_fields(
        ...
        GatewayError::new(Code::InvalidArgument, message)
    ...
    pub fn panicked(domain: &str, method: &str) -> Self {
        GatewayError::new(Code::Internal, "Internal error.").with_error_info(
            "GATEWAY_PANIC",
            domain,
