...
pub fn denied() -> Box<GatewayError> {
    Box::new(
        GatewayError::new(Code::PermissionDenied, "No access to that artist.")
            .ensure_error_info(DOMAIN),
    )
}

/// An `INTERNAL`, standing in for a backend dying mid-stream.
pub fn exploded() -> Box<GatewayError> {
    Box::new(GatewayError::new(Code::Internal, "Backend went away.").ensure_error_info(DOMAIN))
}

/// Encodes an error the way the JSON codec would.
pub fn encode(err: &GatewayError) -> Vec<u8> {
