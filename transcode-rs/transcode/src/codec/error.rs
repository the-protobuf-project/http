...
use crate::error::{Code, Error, FieldViolation};
...
impl CodecError {
    ...
    pub fn into_gateway_error(self, domain: &str, method: &str) -> Error {
        match self {
            CodecError::Malformed { reason } => Error::new(
            ...
            CodecError::Field { path, reason } => Error::invalid_fields(
                vec![FieldViolation {
                    field: path,
                    description: reason,
            ...
            CodecError::UnknownField { path } => Error::invalid_fields(
                vec![FieldViolation {
                    field: path.clone(),
            ...
            CodecError::Encode { reason } => {
                ...
                Error::new(Code::Internal, "Failed to encode the response.").with_error_info(
                    "ENCODE_FAILED",
                    domain,
                    [("method".into(), method.into())],
                )
