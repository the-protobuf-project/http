...
use super::DOMAIN;
use crate::error::{Code, Detail, Error, Help, HelpLink, RetryInfo};
use http::header;
use serde_json::json;

#[test]
fn retry_info_projects_to_retry_after() {
    let err = Error::new(Code::Unavailable, "try later")
...
fn unauthenticated_emits_a_well_formed_challenge() {
    ...
    let err = Error::new(Code::Unauthenticated, "token \"abc\" expired").ensure_error_info(DOMAIN);
...
fn help_links_project_to_link_headers() {
    let err = Error::new(Code::InvalidArgument, "bad request")
