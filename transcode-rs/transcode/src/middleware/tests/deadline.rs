...
/// The error domain these tests build interceptors with.
///
/// Named rather than repeated, so a test that starts asserting on the domain of
/// a `DEADLINE_EXCEEDED` has one place to look for what it should be.
const TEST_DOMAIN: &str = "music.example.com";

#[test]
fn deadline_prefers_the_client_timeout_but_caps_it() {
    let deadline =
        Deadline::new(Duration::from_secs(30), TEST_DOMAIN).with_max(Duration::from_secs(60));
...
fn deadline_forwards_the_budget_to_the_service() {
    ...
    Deadline::new(Duration::from_secs(30), TEST_DOMAIN)
        .on_route(&mut cx)
        .unwrap();

    // So the backend stops working on a call the transcoder has abandoned.
